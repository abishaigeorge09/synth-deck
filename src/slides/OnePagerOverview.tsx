...
type Props = NavOverrides // accepted for slide-registry compatibility; unused
...
function LogoMarquee() {
  // Duplicate enough times that even with only a few logos the strip is
  // always full edge-to-edge. We render two halves: [A B C D E][A B C D E]
  // and animate x from 0 → -50% so the first half scrolls off as the
  // second half scrolls in — seamless infinite loop, no empty gap.
  const reps = 4
...
export function OnePagerOverview(_props: Props) {
  return (
    <div
    ...
    >
      ...
      <div className="relative z-[1] mt-10 flex min-h-0 flex-1 flex-col items-center justify-center">
        ...
        >
          ...
          <div
            className="mt-4 whitespace-nowrap text-center text-[clamp(16px,1.7vw,24px)] italic"
            style={{ fontFamily: THEME.fontSerif, color: THEME.textSecondary }}
          >
            Unlock every signal. Push past every limit.
          </div>
        </motion.div>
        ...
        <motion.div
          ...
          className="mt-12 flex w-full max-w-[1040px] items-start justify-between"
        >
          <TeamCard
